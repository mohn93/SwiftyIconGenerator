...
//const FLUTTER_FONT_TEMPLATE
// =_.template(fs.readFileSync(path.join(TEMPLATES_DIR, 'font/flutter_iconset.tpl'),'utf8'));
const SWIFT_FONT_TEMPLATE = _.template(fs.readFileSync(path.join(TEMPLATES_DIR, 'font/swift_iconset.tpl'), 'utf8'));
const SVG_FONT_TEMPLATE = _.template(fs.readFileSync(path.join(TEMPLATES_DIR, 'font/svg.tpl'), 'utf8'));
...
module.exports = async function fontWorker(taskInfo) {
  ...
  //       }
  // );
  let swift_file = fontname.charAt(0).toLowerCase() + `${fontname}_icons.swift`
        .slice(1)
        .replace(/([A-Z])/g, function (m) {
          return '_' + m.toLowerCase();
        }
  ...
  files = {
    ...
    swift: path.join(taskInfo.tmpDir, swift_file)
  };
  ...
  let swiftOutput = SWIFT_FONT_TEMPLATE(buildFlutterConfig(taskInfo.builderConfig));
  ...
  await mz.fs.writeFile(files.swift, swiftOutput, 'utf8');
